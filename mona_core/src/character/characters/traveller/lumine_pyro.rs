...
pub struct LuminePyroSkillType {
    pub a_dmg1: [f64; 15],
    pub a_dmg2: [f64; 15],
    pub a_dmg3: [f64; 15],
    pub a_dmg4: [f64; 15],
...
pub const LUMINEPYRO_SKILL: LuminePyroSkillType = LuminePyroSkillType {
...
pub const LUMINEPYRO_STATIC_DATA: CharacterStaticData = CharacterStaticData {
    name: CharacterName::LuminePyro,
    internal_name: "LuminePyro",
...
pub struct LuminePyroEffect {
    pub elements: ConfigElements8Multi,
    pub common_data: CharacterCommonData,
}

impl<A: Attribute> ChangeAttribute<A> for LuminePyroEffect {
...
impl LuminePyroDamageEnum {
    pub fn get_element(&self, fumo: bool) -> Element {
        use LuminePyroDamageEnum::*;
        match *self {
...
pub struct LuminePyro;

impl CharacterTrait for LuminePyro {
    const STATIC_DATA: CharacterStaticData = LUMINEPYRO_STATIC_DATA;
    type SkillType = LuminePyroSkillType;
    const SKILL: Self::SkillType = LUMINEPYRO_SKILL;
    type DamageEnumType = LuminePyroDamageEnum;
    type RoleEnum = ();
    ...
    fn damage_internal<D: DamageBuilder>(context: &DamageContext<'_, D::AttributeType>, s: usize, config: &CharacterSkillConfig, fumo: Option<Element>) -> D::Result {
        let s: LuminePyroDamageEnum = num::FromPrimitive::from_usize(s).unwrap();
        let (s1, s2, s3) = context.character_common_data.get_3_skill();

        let (gosoythoth, nightsouls_blessing, active, activated_q) = match *config {
            CharacterSkillConfig::LuminePyro { gosoythoth, nightsouls_blessing, active, activated_q } => (gosoythoth, nightsouls_blessing, active, activated_q),
        ...
        let ratio = match s {
            A1 => LUMINEPYRO_SKILL.a_dmg1[s1],
            A2 => LUMINEPYRO_SKILL.a_dmg2[s1],
            A3 => LUMINEPYRO_SKILL.a_dmg3[s1],
            A4 => LUMINEPYRO_SKILL.a_dmg4[s1],
            A5 => LUMINEPYRO_SKILL.a_dmg5[s1],
            Z1 => LUMINEPYRO_SKILL.z_dmg1[s1],
            Z2 => LUMINEPYRO_SKILL.z_dmg2[s1],
            X1 => LUMINEPYRO_SKILL.x_dmg1[s1],
            X2 => LUMINEPYRO_SKILL.x_dmg2[s1],
            X3 => LUMINEPYRO_SKILL.x_dmg3[s1],
            E1 => LUMINEPYRO_SKILL.e_dmg1[s2],
            EH => LUMINEPYRO_SKILL.e_dmgh[s2],
            E2 => LUMINEPYRO_SKILL.e_dmg2[s2],
            Q => LUMINEPYRO_SKILL.q_dmg[s3],
            C6 => LUMINEPYRO_SKILL.c6_dmg,
            P3A => LUMINEPYRO_SKILL.z_dmg1[s1] + LUMINEPYRO_SKILL.p3_bonus,
            P3B => LUMINEPYRO_SKILL.z_dmg2[s1] + LUMINEPYRO_SKILL.p3_bonus,
            P3T => LUMINEPYRO_SKILL.z_dmg1[s1] + LUMINEPYRO_SKILL.z_dmg2[s1] + 2.0 * LUMINEPYRO_SKILL.p3_bonus,
            _ => 0.0
        };
    ...
    fn new_effect<A: Attribute>(common_data: &CharacterCommonData, config: &CharacterConfig) -> Option<Box<dyn ChangeAttribute<A>>> {
        let elements = match *config {
            CharacterConfig::LuminePyro { elements, .. } => elements,
            _ => ConfigElements8Multi::default(),
        };
        Some(Box::new(LuminePyroEffect {
            elements,
