...
pub struct AetherHydroSkillType {
    pub a_dmg1: [f64; 15],
    pub a_dmg2: [f64; 15],
    pub a_dmg3: [f64; 15],
    pub a_dmg4: [f64; 15],
...
pub const AETHERHYDRO_SKILL: AetherHydroSkillType = AetherHydroSkillType {
...
pub const AETHERHYDRO_STATIC_DATA: CharacterStaticData = CharacterStaticData {
    name: CharacterName::AetherHydro,
    internal_name: "AetherHydro",
...
pub struct AetherHydroEffect {
    pub elements: ConfigElements8Multi,
    pub common_data: CharacterCommonData,
}

impl<A: Attribute> ChangeAttribute<A> for AetherHydroEffect {
...
impl AetherHydroDamageEnum {
    pub fn get_element(&self) -> Element {
        use AetherHydroDamageEnum::*;
        match *self {
...
pub struct AetherHydro;

impl CharacterTrait for AetherHydro {
    const STATIC_DATA: CharacterStaticData = AETHERHYDRO_STATIC_DATA;
    type SkillType = AetherHydroSkillType;
    const SKILL: Self::SkillType = AETHERHYDRO_SKILL;
    type DamageEnumType = AetherHydroDamageEnum;
    type RoleEnum = ();
    ...
    fn damage_internal<D: DamageBuilder>(context: &DamageContext<'_, D::AttributeType>, s: usize, config: &CharacterSkillConfig, fumo: Option<Element>) -> D::Result {
        let s: AetherHydroDamageEnum = num::FromPrimitive::from_usize(s).unwrap();
        let (s1, s2, s3) = context.character_common_data.get_3_skill();

        let (hp_above_half, consumed_hp_times) = match *config {
            CharacterSkillConfig::AetherHydro { hp_above_half, consumed_hp_times } => (hp_above_half, consumed_hp_times),
            _ => (false, 0)
        };
        ...
        if s == P3_HEAL {
            ...
            builder.add_hp_ratio("重击·水狱治疗", AETHERHYDRO_SKILL.p3_heal_ratio);
            return builder.heal(&context.attribute);
        }

        // 重击·水狱：生命≥50%时额外每段提升100%攻击力
        let p3_hp_extra = if hp_above_half { AETHERHYDRO_SKILL.p3_hp_bonus } else { 0.0 };

        if s == ED {
            if hp_above_half {
                builder.add_hp_ratio("充盈伤害额外倍率", AETHERHYDRO_SKILL.e_dmg_ad[s2]);
            }
        }
        ...
        let ratio = match s {
            A1 => AETHERHYDRO_SKILL.a_dmg1[s1],
            A2 => AETHERHYDRO_SKILL.a_dmg2[s1],
            A3 => AETHERHYDRO_SKILL.a_dmg3[s1],
            A4 => AETHERHYDRO_SKILL.a_dmg4[s1],
            A5 => AETHERHYDRO_SKILL.a_dmg5[s1],
            Z1 => AETHERHYDRO_SKILL.z_dmg1[s1],
            Z2 => AETHERHYDRO_SKILL.z_dmg2[s1],
            X1 => AETHERHYDRO_SKILL.x_dmg1[s1],
            X2 => AETHERHYDRO_SKILL.x_dmg2[s1],
            X3 => AETHERHYDRO_SKILL.x_dmg3[s1],
            E => AETHERHYDRO_SKILL.e_dmg[s2],
            ED => AETHERHYDRO_SKILL.e_dmg_d[s2],
            EST => AETHERHYDRO_SKILL.e_dmg_st[s2],
            Q => AETHERHYDRO_SKILL.q_dmg[s3],
            P3A => AETHERHYDRO_SKILL.z_dmg1[s1] + AETHERHYDRO_SKILL.p3_bonus + p3_hp_extra,
            P3B => AETHERHYDRO_SKILL.z_dmg2[s1] + AETHERHYDRO_SKILL.p3_bonus + p3_hp_extra,
            P3T => AETHERHYDRO_SKILL.z_dmg1[s1] + AETHERHYDRO_SKILL.z_dmg2[s1] + 2.0 * (AETHERHYDRO_SKILL.p3_bonus + p3_hp_extra),
            _ => 0.0
        };
    ...
    fn new_effect<A: Attribute>(common_data: &CharacterCommonData, config: &CharacterConfig) -> Option<Box<dyn ChangeAttribute<A>>> {
        let elements = match *config {
            CharacterConfig::AetherHydro { elements, .. } => elements,
            _ => ConfigElements8Multi::default(),
        };
        Some(Box::new(AetherHydroEffect {
            elements,
