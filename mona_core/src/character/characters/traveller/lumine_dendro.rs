...
pub struct LumineDendroSkillType {
    pub a_dmg1: [f64; 15],
    pub a_dmg2: [f64; 15],
    pub a_dmg3: [f64; 15],
    pub a_dmg4: [f64; 15],
...
pub const LUMINEDENDRO_SKILL: LumineDendroSkillType = LumineDendroSkillType {
...
pub const LUMINEDENDRO_STATIC_DATA: CharacterStaticData = CharacterStaticData {
    name: CharacterName::LumineDendro,
    internal_name: "LumineDendro",
...
pub struct LumineDendroEffect {
    pub elements: ConfigElements8Multi,
    pub common_data: CharacterCommonData,
}

impl<A: Attribute> ChangeAttribute<A> for LumineDendroEffect {
...
impl LumineDendroDamageEnum {
    pub fn get_element(&self) -> Element {
        use LumineDendroDamageEnum::*;
        match *self {
...
pub struct LumineDendro;

impl CharacterTrait for LumineDendro {
    const STATIC_DATA: CharacterStaticData = LUMINEDENDRO_STATIC_DATA;
    type SkillType = LumineDendroSkillType;
    const SKILL: Self::SkillType = LUMINEDENDRO_SKILL;
    type DamageEnumType = LumineDendroDamageEnum;
    type RoleEnum = ();
    ...
    fn damage_internal<D: DamageBuilder>(context: &DamageContext<'_, D::AttributeType>, s: usize, config: &CharacterSkillConfig, fumo: Option<Element>) -> D::Result {
        let s: LumineDendroDamageEnum = num::FromPrimitive::from_usize(s).unwrap();
        let (s1, s2, s3) = context.character_common_data.get_3_skill();

        let overflowing_lotuslight_count = match config {
            CharacterSkillConfig::LumineDendro { overflowing_lotuslight_count } => *overflowing_lotuslight_count,
            _ => 0
        };
        ...
        let ratio = match s {
            A1 => LUMINEDENDRO_SKILL.a_dmg1[s1],
            A2 => LUMINEDENDRO_SKILL.a_dmg2[s1],
            A3 => LUMINEDENDRO_SKILL.a_dmg3[s1],
            A4 => LUMINEDENDRO_SKILL.a_dmg4[s1],
            A5 => LUMINEDENDRO_SKILL.a_dmg5[s1],
            Z1 => LUMINEDENDRO_SKILL.z_dmg1[s1],
            Z2 => LUMINEDENDRO_SKILL.z_dmg2[s1],
            X1 => LUMINEDENDRO_SKILL.x_dmg1[s1],
            X2 => LUMINEDENDRO_SKILL.x_dmg2[s1],
            X3 => LUMINEDENDRO_SKILL.x_dmg3[s1],
            E => LUMINEDENDRO_SKILL.e_dmg[s2],
            QC => LUMINEDENDRO_SKILL.q_dmgc[s3],
            QE => LUMINEDENDRO_SKILL.q_dmge[s3],
            P3A => LUMINEDENDRO_SKILL.z_dmg1[s1] + LUMINEDENDRO_SKILL.p3_bonus,
            P3B => LUMINEDENDRO_SKILL.z_dmg2[s1] + LUMINEDENDRO_SKILL.p3_bonus,
            P3T => LUMINEDENDRO_SKILL.z_dmg1[s1] + LUMINEDENDRO_SKILL.z_dmg2[s1] + 2.0 * LUMINEDENDRO_SKILL.p3_bonus,
            P3L1 | P3L2 => LUMINEDENDRO_SKILL.p3_lotus_bonus,
            _ => 0.0
        };
    ...
    fn new_effect<A: Attribute>(common_data: &CharacterCommonData, config: &CharacterConfig) -> Option<Box<dyn ChangeAttribute<A>>> {
        let elements = match *config {
            CharacterConfig::LumineDendro { elements } => elements,
            _ => ConfigElements8Multi::default(),
        };
        Some(Box::new(LumineDendroEffect {
            elements,
