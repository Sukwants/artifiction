...
impl CharacterTrait for AetherGeo {
    ...
    fn change_attribute<A: Attribute>(attribute: &mut A, common_data: &CharacterCommonData, skill_config: &CharacterSkillConfig) {
        ...
        let shield_buff = match *skill_config {
            CharacterSkillConfig::AetherGeo { shield_buff } => shield_buff,
            _ => false,
        };
    ...
    fn damage_internal<D: DamageBuilder>(context: &DamageContext<'_, D::AttributeType>, s: usize, config: &CharacterSkillConfig, fumo: Option<Element>) -> D::Result {
        ...
        let ratio = match s {
            ...
            P3A => AETHERGEO_SKILL.z_dmg1[s1] + AETHERGEO_SKILL.p3_bonus,
            P3B => AETHERGEO_SKILL.z_dmg2[s1] + AETHERGEO_SKILL.p3_bonus,
            P3T => AETHERGEO_SKILL.z_dmg1[s1] + AETHERGEO_SKILL.z_dmg2[s1] + 2.0 * AETHERGEO_SKILL.p3_bonus,
            _ => 0.0
        };
    ...
    fn new_effect<A: Attribute>(common_data: &CharacterCommonData, config: &CharacterConfig) -> Option<Box<dyn ChangeAttribute<A>>> {
        let elements = match *config {
            CharacterConfig::AetherGeo { elements } => elements,
