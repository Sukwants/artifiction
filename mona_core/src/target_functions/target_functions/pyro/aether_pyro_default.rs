...
use crate::character::characters::AetherPyro;
...
pub struct AetherPyroDefaultTargetFunction {
    pub vaporize_rate: f64,
    pub melt_rate: f64,
    pub gosoythoth: bool,
}

impl TargetFunctionMetaTrait for AetherPyroDefaultTargetFunction {
    ...
    const META_DATA: TargetFunctionMeta = TargetFunctionMeta {
        ...
        description: locale!(
            zh_cn: "旅行者空-火前台每轮总伤害",
            en: "Traveller Aether-Pyro Total DMG per Round"
        ),
        tags: "输出",
        four: TargetFunctionFor::SomeWho(CharacterName::AetherPyro),
    ...
    fn create(_character: &CharacterCommonData, _weapon: &WeaponCommonData, config: &TargetFunctionConfig) -> Box<dyn TargetFunction> {
        let (vaporize_rate, melt_rate, gosoythoth) = match *config {
            TargetFunctionConfig::AetherPyroDefault { vaporize_rate, melt_rate, gosoythoth } => (vaporize_rate, melt_rate, gosoythoth),
            _ => (0.0, 0.0, false)
        };
        Box::new(AetherPyroDefaultTargetFunction {
            vaporize_rate,
            melt_rate,
            gosoythoth,
        })
...
impl TargetFunction for AetherPyroDefaultTargetFunction {
    ...
    fn target(&self, attribute: &SimpleAttributeGraph2, character: &Character<SimpleAttributeGraph2>, weapon: &Weapon<SimpleAttributeGraph2>, artifacts: &[&Artifact], enemy: &Enemy) -> f64 {
        ...
        let config = CharacterSkillConfig::AetherPyro {
        ...
        type DmgType = <AetherPyro as CharacterTrait>::DamageEnumType;
        let c_count = context.character_common_data.constellation;

        let dmg_q = calc_damage(AetherPyro::damage::<SimpleDamageBuilder>(&context, DmgType::Q, &CharacterSkillConfig::AetherPyro {
            gosoythoth: self.gosoythoth,
            nightsouls_blessing: true,
            active: true,
            activated_q: false,
        ...
        let dmg_e = calc_damage(AetherPyro::damage::<SimpleDamageBuilder>(&context, DmgType::E1, &config, None));

        let dmg_a = calc_damage(AetherPyro::damage::<SimpleDamageBuilder>(&context, DmgType::A1, &config, None))
            + calc_damage(AetherPyro::damage::<SimpleDamageBuilder>(&context, DmgType::A2, &config, None))
            + calc_damage(AetherPyro::damage::<SimpleDamageBuilder>(&context, DmgType::A3, &config, None));
        
        let dmg_z = calc_damage(AetherPyro::damage::<SimpleDamageBuilder>(&context, DmgType::Z1, &config, None))
            + calc_damage(AetherPyro::damage::<SimpleDamageBuilder>(&context, DmgType::Z2, &config, None));
