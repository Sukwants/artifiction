...
pub const EMPTY_LOCALE: I18nLocale = I18nLocale {
    zh_cn: "",
    en: "",
    ja: "",
};
...
macro_rules! locale {
    ($($name:ident : $value:expr),* $(,)?) => {
        crate::common::i18n::I18nLocale {
            $(
                $name: $value,
            )*
            ..$crate::common::i18n::EMPTY_LOCALE
        }
    };
}

macro_rules! hit_n_dmg {
    (1) => {
    ...
    (1, $n:expr) => {
        locale!(
            zh_cn: concat!("一段伤害-", $n),
            en: concat!("1-Hit DMG-", $n)
        )
    };
    (2, $n:expr) => {
        locale!(
            zh_cn: concat!("二段伤害-", $n),
            en: concat!("2-Hit DMG-", $n)
        )
    };
    (3, $n:expr) => {
        locale!(
            zh_cn: concat!("三段伤害-", $n),
            en: concat!("3-Hit DMG-", $n)
        )
    };
    (4, $n:expr) => {
        locale!(
            zh_cn: concat!("四段伤害-", $n),
            en: concat!("4-Hit DMG-", $n)
        )
    };
    (5, $n:expr) => {
        locale!(
            zh_cn: concat!("五段伤害-", $n),
            en: concat!("5-Hit DMG-", $n)
        )
    };
    (6, $n:expr) => {
        locale!(
            zh_cn: concat!("六段伤害-", $n),
            en: concat!("6-Hit DMG-", $n)
        )
    };
}

macro_rules! plunging_dmg {
    (1) => {
...
pub(crate) use hit_n_dmg;
pub(crate) use plunging_dmg;
pub(crate) use charged_dmg;