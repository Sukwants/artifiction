...
pub struct BuffLuminePyroC1 {
    pub nightsouls_blessing: bool,
}

impl<A: Attribute> Buff<A> for BuffLuminePyroC1 {
    fn change_attribute(&self, attribute: &mut A) {
        attribute.set_value_by(AttributeName::BonusBase, "荧-火「流光的星火」", if self.nightsouls_blessing { 0.15 } else { 0.06 });
    }
}

impl BuffMeta for BuffLuminePyroC1 {
    #[cfg(not(target_family = "wasm"))]
    const META_DATA: BuffMetaData = BuffMetaData {
        name: BuffName::LuminePyroC1,
        name_locale: locale!(
            zh_cn: "荧-火-「流光的星火」",
            en: "Lumine-Pyro-Starfire's Flowing Light"
        ),
        image: BuffImage::Avatar(CharacterName::LuminePyro),
        ...
        from: BuffFrom::Character(CharacterName::LuminePyro),
    };
    ...
    fn create<A: Attribute>(b: &BuffConfig) -> Box<dyn Buff<A>> {
        let nightsouls_blessing = match *b {
            BuffConfig::LuminePyroC1 { nightsouls_blessing } => nightsouls_blessing,
            _ => false
        };
        Box::new(BuffLuminePyroC1 {
            nightsouls_blessing,
        })
    }
}

pub struct BuffLuminePyroC2 {
    pub stack: usize,
}

impl<A: Attribute> Buff<A> for BuffLuminePyroC2 {
    fn change_attribute(&self, attribute: &mut A) {
        attribute.set_value_by(AttributeName::ResMinusPyro, "荧-火「长明的烛火」", self.stack as f64 * 0.2);
    }
}

impl BuffMeta for BuffLuminePyroC2 {
    #[cfg(not(target_family = "wasm"))]
    const META_DATA: BuffMetaData = BuffMetaData {
        name: BuffName::LuminePyroC2,
        name_locale: locale!(
            zh_cn: "荧-火-「长明的烛火」",
            en: "Lumine-Pyro-Ever-Lit Candle"
        ),
        image: BuffImage::Avatar(CharacterName::LuminePyro),
        genre: BuffGenre::Character,
        description: Some(locale!(
    ...
    fn create<A: Attribute>(b: &BuffConfig) -> Box<dyn Buff<A>> {
        let stack = match *b {
            BuffConfig::LuminePyroC2 { stack } => stack,
            _ => 0
        };
        Box::new(BuffLuminePyroC2 {
            stack
        })
    }
}