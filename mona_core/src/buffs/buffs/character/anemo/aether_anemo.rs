...
pub struct BuffAetherAnemoC6 {
    pub has_elemental_absorption: bool,
    pub elemental_absorption: Element,
}

impl<A: Attribute> Buff<A> for BuffAetherAnemoC6 {
    fn change_attribute(&self, attribute: &mut A) {
        attribute.set_value_by(AttributeName::ResMinusAnemo, "空-风「纠缠的信风」", 0.20);
        if self.has_elemental_absorption {
            attribute.set_value_by(AttributeName::res_minus_name_by_element(self.elemental_absorption), "空-风「纠缠的信风」", 0.20);
        }
    }
}

impl BuffMeta for BuffAetherAnemoC6 {
    #[cfg(not(target_family = "wasm"))]
    const META_DATA: BuffMetaData = BuffMetaData {
        name: BuffName::AetherAnemoC6,
        name_locale: locale!(
            zh_cn: "空-风-「纠缠的信风」",
            en: "Aether-Anemo-Intertwined Winds"
        ),
    ...
    fn create<A: Attribute>(b: &BuffConfig) -> Box<dyn Buff<A>> {
        let (has_elemental_absorption, elemental_absorption) = match *b {
            BuffConfig::AetherAnemoC6 { has_elemental_absorption, elemental_absorption } => (has_elemental_absorption, elemental_absorption),
            _ => (false, Element::Cryo)
        };
        Box::new(BuffAetherAnemoC6 {
