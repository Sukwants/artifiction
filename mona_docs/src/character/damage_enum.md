...
一般情况下，两个分开书写但不会同时出现、且紧密关联的技能（如一个技能伤害在某些情况下替换另一个技能伤害），应当使用同一个 DamageEnum，并根据角色配置、技能配置在 `damage_internal` 中进行判断，返回不同的伤害倍率。
...
对于触发描述位于命座中且未额外说明伤害类型的伤害，优先按倍率所在条目判定：倍率位于普通攻击、元素战技或元素爆发中时，分别视为对应类型；否则，若伤害名称与其中某种伤害名称大致相同，或前者是后者的变体，也视为该类型。仍无法判定时，技能类型与触发该伤害的实际操作或施放的技能、伤害保持一致；同一种伤害可能产生两种不同技能类型时，必须拆分为多个 `DamageEnum` 变体。AI 若难以判断该伤害由何种实际操作、技能或伤害触发，必须请求人类确认。
...
擢升反应是与普通攻击、重击、下落攻击、元素战技、元素爆发并列的独立技能类型，不匹配上述类型专属的基础伤害、伤害加成、暴击率或暴击伤害。
若是否为擢升反应取决于配置或状态，`get_skill_type` 必须接收相同条件并返回最终类型，后续 `builder.elevative` 统一传入该返回值。
...
例如，当命座伤害按上述规则可能由不同技能来源触发时（如命座4的弹射伤害既可能由元素战技触发也可能由元素爆发触发），应将该伤害拆分为多个 `DamageEnum` 变体，分别归入对应技能的 `skill_map` 中。拆分后在 `get_skill_type` 中也应分别返回对应的 `SkillType`：
