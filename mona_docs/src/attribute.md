...
- 特别的，星超导伤害的基础倍率与月曜反应不同，由冰、雷元素附着次数决定，因此对于任何会触发星超导伤害的角色，需要通过角色配置来获取当前附着次数，附着次数达到 $0 \sim 12$ 次时的额外基础倍率从 `ElevativeReaction::STELLAR_CONDUCT_EXTRA_COEFFICIENT` 数组中获取，该额外基础倍率应当通过 `ElevativeCoefficient` 部分进行加成，且 `ElevativeCoefficient` 仅会受到上述描述的加成效果影响，其他加成效果不应影响该部分。
